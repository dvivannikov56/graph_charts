...
//console.log($('input.highcharts-range-selector:eq(0)').datepicker("setDate",new Date(dateText)));
...
series: [{
    ...
    headerFormat:'<span style="font-weight: normal;" class="date">{point.key}</span>',
	 pointFormatter: function () {
            return '<span class="series-name'+this.series.index+'">'+this.series.name+':</span><b class="point'+this.series.index+'">'+Highcharts.numberFormat(this.y, 2)+ ' руб.</b>';
        },
   shared: true,
    split: false,
    enabled: true,
...
$('g.highcharts-range-selector-group > g.highcharts-input-group > g:nth-child(4)').append(txt2);
	 ...
	 $('.highcharts-legend-item:eq(0)> text').text('Стоимость ПАЯ');
...
for (var i = 0; i < data[0][0].length; i++) {
	enabled_date[i] = data[0][0][i].toLocaleDateString();
}
...
var max = chart.yAxis[0].dataMax;
 ...
 var bdohod=(max-min).toFixed(2);
  ...
  $('<span class="dohod">'+ bdohod+' руб ('+dohod+'%)<span>').appendTo(".highcharts-title");
	   ...
	   var last_date = new Date(data[0][0][data[0][0].length-1]);
   ...
   $('#year5').click(function(e) {
    ...
    name: "Бенчмарк",
    data: ohlc2,
    pointFormat: '<span style="color:#92d2f1">{series.name}:</span><br><b style="font-size: 18px;color:#92d2f1">{point.y}</b><br/>',
     fillColor: {
            linearGradient: {
                x1: 0,
                y1: 0,
  ...
  // var pnt =  document.querySelector('path.highcharts-graph');
...
var buttons = chart.rangeSelector.buttons;
  ...
  buttons.forEach(btn => {
    var value = parseFloat(btn.text.element.attributes.x.value),
      labelWidth = btn.text.element.getBBox().width;

    btn.text.element.attributes.x.value = 20;
  });
   var value = parseFloat(chart.rangeSelector.buttons[7].text.element.attributes.x.value);
...
console.log(extreme.dataMin + ' ' + extreme.dataMax);
    ...
    $('#print').click(function () {
        ...
        var divToPrint=document.querySelector(".pais");
   newWin= window.open(' ');
...
<div class="pr">
    <div id="container" class="graph_container cnt_one"></div>
    <div class="periods">
